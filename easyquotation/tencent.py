...
class Tencent(basequotation.BaseQuotation):
    ...
    def format_hkstock_data(self, stock, prefix=False):
        stock_code = (
        ...
        stock_data = {
            ...
            "成交量(股)": float(stock[36]),
            "成交额(HKD)": float(stock[37]),
    ...
    def format_A_stock_data(self, stock, prefix=False):
        ...
        stock_data = {
            ...
            "low": float(stock[34]),
            "价格/成交量(手)/成交额": stock[35],
            "成交量(股)": int(stock[36]) * 100,
            "成交额(元)": float(stock[37]) * 10000,
            "turnover": self._safe_float(stock[38]),
            "PE": self._safe_float(stock[39]),
            "unknown": stock[40],
            "high_2": float(stock[41]),  # 意义不明
    ...
    def format_response_data(self, rep_data, prefix=False):
        ...
        for stock_detail in stock_details:
            ...
            if stock[0].startswith('v_hk'):
                stock_code, stock_data = self.format_hkstock_data(stock, prefix)
            else:
                stock_code, stock_data = self.format_A_stock_data(stock, prefix)
